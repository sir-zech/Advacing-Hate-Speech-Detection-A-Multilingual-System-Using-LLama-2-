                ...
                if choice == "Record":
                    ...
                    if st.button("START RECORDING"):
                        ...
                        if transcript:
                            ...
                            hate_speech_chunks = 0
                            for chunk in chunks:
                                prompt = convert_to_prompt(chunk)
                                predictions = generate_predictions(model, tokenizer, [prompt])
                                if predictions[0] == "Hate Speech":
                                    hate_speech_chunks += 1
                            
                            # Calculate hate speech percentage
                            total_chunks = len(chunks)
                            hate_speech_percentage = (hate_speech_chunks / total_chunks) * 100 if total_chunks > 0 else 0
                            
                            if hate_speech_chunks > 0:
                                overall_prediction = "Hate Speech Detected"
                                st.markdown(f'<p style="color:red; font-size:20px;">{overall_prediction}</p>', unsafe_allow_html=True)
                                st.progress(hate_speech_percentage / 100)
                                st.markdown(f'<p style="font-size:18px;">Hate Speech Content: <span style="color:red; font-weight:bold;">{hate_speech_percentage:.2f}%</span> ({hate_speech_chunks} out of {total_chunks} chunks)</p>', unsafe_allow_html=True)
                            else:
                                ...
                                st.progress(0.0)
                                st.markdown(f'<p style="font-size:18px;">Hate Speech Content: <span style="color:green; font-weight:bold;">0.00%</span> (0 out of {total_chunks} chunks)</p>', unsafe_allow_html=True)
